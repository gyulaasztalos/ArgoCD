...
8. **Create Endpoints for the etcd ServiceMonitor:**
`kubectl apply -f apps/monitoring/post-install/kube-etcd-endpoints.yaml`
Without it the etcd scrape target never appears, so the etcd dashboard and all 15 etcd alerts stay silently empty. Note this one is an **Endpoints** object, not an EndpointSlice — Prometheus leaves `serviceDiscoveryRole` unset, which defaults to `Endpoints`, and Kubernetes only mirrors Endpoints → EndpointSlice, never the reverse.
Requires `--etcd-expose-metrics` in the k3s server args, otherwise port 2381 only listens on 127.0.0.1 and the scrape is refused.
...
2. **EndpointSlices**: ArgoCD exclude handling of Endpoint and EndpointSlice manifests (for a good reason). Generally EndpointSlices are auto-generated by Kubernetes, based on the Service. However if the Service has no selector the EndpointSlice has to be created manually (outside of the GitOps workflow). Two cases: `unifi-redirect.yaml`, where we use traefik in the cluster to apply the wildcard certificate on an external page; and `kube-etcd-endpoints.yaml`, because k3s runs etcd inside the k3s server process rather than as a pod, so the chart's kubeEtcd Service selector matches nothing. Setting `kubeEtcd.endpoints` in values.yaml makes the chart render the object correctly — ArgoCD just drops it, so it must be applied by hand. **The failure is silent**: the Service exists, the ServiceMonitor exists, and the only symptom is an empty dashboard and alerts that can never fire.
